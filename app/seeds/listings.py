...
def seed_listings():
    test1 = Listing(userId = 1,description='barely even took it out of the box',price=100,condition='Factory New',category='Keyboard',subcategory='tkl',image='https://i.pcmag.com/imagery/roundups/00aE5K1WsINRBBPjHLRHbXV-1..1569470766.jpg')
    test2 = Listing(userId = 2,description='built this from scratch all switches are lubed',price=250,condition='Minimal Wear',category='Keyboard',subcategory='full',image='https://thegadgetflow.com/wp-content/uploads/2021/05/Top-5-mechanical-keyboards-for-your-workspace-blog-featured.jpeg')
    ...
    test4 = Listing(userId = 1,description='the stabilizers are broken so some repair needed',price=180,condition='Well-Worn',category='Keyboard',subcategory='65',image='https://images.indianexpress.com/2021/06/Corsair-Mechanical-Keyboard.jpg')
    ...
    test6 = Listing(userId = 3,description='just upgraded and built a new keyboard so i dont need this one anymore',price=500,condition='Minimal Wear',category='Keyboard',subcategory='60',image='https://cdn.shopify.com/s/files/1/0059/0630/1017/products/Keychron-K2-hot-swappable-wireless-mechanical-keyboard-for-Mac-Windows-iOS-Gateron-switch-blue-with-type-C-RGB-white-backlight_1800x1800.jpg?v=1604558657')
    test7 = Listing(userId = 1,description='never had an issues with it and the stabs are lubed',price=89,condition='Field-Tested',category='Keyboard',subcategory='other',image='https://cdn.shopify.com/s/files/1/0003/0182/6108/products/20200814_113759_grande.jpg?v=1604264946')
    test8 = Listing(userId = 2,description='the case has seen better days but all the internals are good',price=212,condition='Well-Worn',category='Keyboard',image='https://cdn.vox-cdn.com/thumbor/i4CZ-fGvDjbidhcTFQ9WLJZszYI=/0x0:2040x1360/1200x0/filters:focal(0x0:2040x1360):no_upscale()/cdn.vox-cdn.com/uploads/chorus_asset/file/16351678/pseawell_190614_3491_0002.jpg')
    ...
    test10 = Listing(userId = 1,description='brand new but i dont like clicky switches so my loss is your gain',price=222,condition='Factory New',category='Keyboard',image='https://m.media-amazon.com/images/I/71nRfZNacyL._AC_SL1500_.jpg')
    ...
    test12 = Listing(userId = 2,description='built this from scratch all switches are lubed',price=250,condition='Minimal Wear',category='Keyboard',image='https://media.wired.com/photos/60be799feaa210b2096c93fa/master/pass/games_keyboards_S2Y202.jpg')
    test13 = Listing(userId = 3,description='i dont like the switches and im too lazy to change them',price=75,condition='Field-Tested',category='Keyboard',image='https://images.anandtech.com/doci/12061/CorsairK100CarB_678x452.jpg')
    test14 = Listing(userId = 1,description='the stabilizers are broken so some repair needed',price=180,condition='Well-Worn',category='Keyboard',image='https://pisces.bbystatic.com/image2/BestBuy_US/images/products/6421/6421508_sd.jpg')
    ...
    test16 = Listing(userId = 3,description='just upgraded and built a new keyboard so i dont need this one anymore',price=500,condition='Minimal Wear',category='Keyboard',image='https://c1.neweggimages.com/ProductImage/A9HJD200928OHVPH.jpg')
